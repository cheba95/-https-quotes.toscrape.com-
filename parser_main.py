...
def parse_page(page):
    raw_quotes = page.find_all('div', class_='quote')# находим все блоки цитат с элементами "текст", "автор" и "тэги"
    for rq in raw_quotes:# идем циклом по всем найденным блокам цитат, сохраняем данные
        quote_text = rq.find('span', class_='text').text.strip('“”')  # получаем текст цитаты.
        # срезаем кавычки, т.к. вместо них в JSON файле код "\u201c и \u201d, смысл не теряется, читаемость улучшается
        author = rq.find('small', class_='author').text # получаем автора
        if not author in authors_info: # проверяем, есть ли автор в словаре авторов и сведений о них. Если нет - добавляем информацию
            ...
            #для поиска
            birth_date = bio_page.find('span', class_='author-born-date').text # находим и сохраняем дату рождения автора
            birth_place = bio_page.find('span', class_='author-born-location').text # находим и сохраняем место рождения автора
            description = bio_page.find('div', class_='author-description').text.strip() # находим и сохраняем сведения об авторе
        ...
        raw_tags = rq.find_all('a', class_='tag') #находим и сохраняем тэги
...
next_page_check = page.find('li', class_='next') # проверяем, если ли на веб-странице переход к следующей веб-странице

while next_page_check is not None: # запускаем цикл обработки всех страниц сайта с цитатами, пока есть кнопка перехода к следующей странице
    next_page_url = url + next_page_check.find('a', href=True)['href'] # создаем адрес следующей страницы
    next_page = requests.get(next_page_url,headers=headers) # делаем запрос к новой страницее и сохраняем ответ 
...
raw_top_tags = page.find('div', class_='col-md-4 tags-box') # находим топ-10 тэгов 
top_tags_list = [i.text for i in raw_top_tags.find_all('a', class_='tag')] # создаем список топ-10 тэгов
top_tags = {key: value for key, value in enumerate(top_tags_list, 1)} # сохраняем топ-10 тэгов как словарь. Ключ - порядковый номер, 
#тэг - значение
